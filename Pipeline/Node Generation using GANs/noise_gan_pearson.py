...
result = torch.load("experiment/embedding models/PICKLE/distmult_model.pth",
                    map_location=device)
...
class OriginalDiscriminator(nn.Module):
    ...
    def forward(self, x, feature=False, cuda=False, first=False):
        ...
        noise = torch.randn(x.size()) * \
            0.05 if self.training else torch.Tensor([0])
        if cuda:
            noise = noise.cuda()
...
if is_incomplete:
    ...
    # Calculate Pearson correlation instead of cosine similarity
    def pearson_similarity(vec1, vec2):
        return np.corrcoef(vec1, vec2)[0, 1]

    existing_node_features = np.array(
        [data['feature'] for _, data in G.nodes(data=True)])
    similarities = np.array([pearson_similarity(
        generated_feature, feature) for feature in existing_node_features])
